...
// header('Content-Disposition:attachment;filename=TEC_Directory.pdf');
    ...
    $dir_extract = "SELECT Surname as LAST, Name_1 as HIM, Name_2 as HER, Address as ADDRESS, Address2 as ADDRESS2, City as CITY, State as STATE, Zip as ZIP, Phone_Home as HOME_PHONE, Phone_Cell1 as HIS_CELL, Phone_Cell2 as HER_CELL, Email_1 as HIS_EMAIL, Email_2 as HER_EMAIL FROM " . $_SESSION['dirtablename'] . " where Status = '1' order by Surname";
    ...
    if($dir_extract_count > 0)
    {
        ...
        {
            $pdfoutput[$i] = $row['LAST'] . ';' . $row['HIM'] . ';' . $row['HER'] . ';' . $row['ADDRESS'] . ';' . $row['ADDRESS2'] . ';' . $row['CITY'] . ';' . $row['STATE'] . ';' . $row['ZIP'] . ';' . $row['HOME_PHONE'] . ';' . $row['HIS_CELL'] . ';' . $row['HER_CELL'] . ';' . $row['HIS_EMAIL'] . ';' . $row['HER_EMAIL'];
            // echo "<script language='javascript'>";
