...
// Query used to populate My Prayer DataTable listing in Popup
		...
		$myprayerquery = "SELECT p.create_date AS prayerupdatedate, p.name AS fullname, m.Name_1 AS firsthim, m.Name_2 AS firsther, m.Surname AS last, p.prayer_id AS prayerid, p.title AS prayertitle, p.prayer_text AS prayertext, p.pray_praise AS praypraise, p.updated AS updatereq, p.answer AS prayanswer FROM " . $_SESSION['prayertable'] . " p INNER JOIN " . $_SESSION['dirtablename'] . " m on m.idDirectory = p.owner_id WHERE p.visible = '3' and p.status = '1' and p.approved='1' and p.owner_id = '" . $_SESSION['user_id'] . "' ORDER BY p.create_date DESC";
...
//			echo "no prayer data";
			...
			$buildjson = array($noprayer, $noprayer, $noprayerecho, $noprayer, $noprayer, $noprayer, $noprayer);
